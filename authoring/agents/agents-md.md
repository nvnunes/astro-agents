...
- Use the file to apply applicable shared prompts or deeper project documentation, not to duplicate them.
...
- Project bootstrap: prioritize a minimal route into the shared prompt library or an explicit project-level opt-out.
- Prompt family dispatcher or selector: prioritize broad dispatch and prompt selection, not substantive prompt behavior.
- Project-level brief: prefer minimal operational guidance first, usually broad routing plus source-of-truth references. Add inline local architecture, contracts, workflow, testing expectations, or review criterion only when that extra guidance is materially useful during work.
