...
A named documentation surface pattern that identifies the expected structure and emphasis of a repo’s human-facing documentation. It can map to corresponding review workflows implemented either by the shared validation library or by higher-authority workspace- or repo-local prompt files.
...
- `control flow`: Avoid this as a broad repo-runtime label when `Route`, `Workflow`, `Handoff`, or `Orchestration` would be more exact.
...
- `override`: Avoid this as a broad repo-runtime term. Prefer higher-priority `Instructions`, superseding file, or plain language such as `replace` or `supersede`. Reserve `AGENTS.override.md` for the Codex filename.
