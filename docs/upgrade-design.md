...
For large or multi-session upgrades, it can help to save the user-approved overall plan to `docs/upgrade-plan.md`.
...
Do not treat `docs/upgrade-plan.md` as required for normal upgrades. Recommend it only when the scope, duration, or likely thread handoff makes continuity materially useful.
