...
2. If no target location can be inferred from an explicit path, repo-local `Research Logs` routing, or chat context, ask before creating files.
...
10. Update repo-local `Research Logs` routing in `AGENTS.md` when the downstream repo has a `## Research Logs` section or another clear research-log theme registry. If no registry exists, ask before adding one. A registry entry should map the theme name to the `<theme>.md` file and matching `<theme>/` folder.
...
- whether repo-local `Research Logs` routing was updated
