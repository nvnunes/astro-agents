...
Read your assigned plan and any child files it assigns you to execute. Execute
only that scope; do not read plans assigned to other Tasks.
Keep working detail and human interaction in your own Task. The main agent
owns its own plan and coordination between Tasks.
...
- Do not continue gated work or send a final handoff while awaiting human input.
...
- State whether prescribed checks and reviews passed, failed, or were not run;
  reference evidence supporting completion and identify unverified requirements.
