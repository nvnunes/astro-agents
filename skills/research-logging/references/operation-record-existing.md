# Record Existing Entry Instructions

Use this file when continuing or revising an existing entry.

`Record Existing` updates one existing entry document without changing
unrelated entry content.

## Procedure

1. Resolve the target entry from the entry ID, path, topic, date, current chat
   context, log summary, entry folder names, search results, or existing links
   before editing.
...
new one. Resolving the destination does not authorize moving earlier material.

When researcher-provided state resolves a TODO or provisional marker, replace
or remove that marker in the affected section. Do not infer that validation,
approval, or another pending action occurred.
