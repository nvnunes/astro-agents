...
def active_evidence_version(summary: Path) -> str:
    ...
    if pending_root.exists() and (
        pending_root.is_symlink()
        or not pending_root.is_dir()
        or any(pending_root.iterdir())
    ):
        _fail(
    ...
    legacy = sorted(log_root.rglob("evidence.csv")) if log_root.is_dir() else []
    if legacy:
        _fail(
            ...
            str(summary),
            {
                ...
                "evidence_json": len(list(log_root.rglob("evidence.json"))),
            },
