...
## Purpose
Use this style as the common base for human-facing technical documents. Apply it to documents meant to be read directly by collaborators, maintainers, reviewers, contributors, or future users, then apply a more specific document-type guide on top when one exists.
...
- For internal file references inside the active project, prefer project-root-relative paths such as `docs/architecture.md` or `AGENTS.md`. When referencing a shared skill directly, use the path convention already established by the target project.
