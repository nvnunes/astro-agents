...
## Purpose
Use this prompt to review a repo's current agent surface against `docs/upgrade-design.md` and recommend a practical upgrade path.

Use it when the user wants to:

- upgrade a repo through the shared review-first path
- review a repo for upgrade readiness
- propose how to group the work for a repo
- assess a repo against the shared upgrade design

Keep this prompt read-only. It reviews the repo and recommends next moves, but it does not perform editing work and it does not create repo-local upgrade artifacts.
...
- optional focus areas such as repo entry surface, source-of-truth docs, environment and validation support, public documentation surface, or how to group the work
...
Ground the assessment in the repo's current surface and `docs/upgrade-design.md`.
...
- describe the repo's current public or private docs setup in plain language rather than relying on `documentation surface profile` or raw profile labels alone
...
- recommend keeping or changing that setup only when current repo evidence supports the recommendation
...
- avoid lead-ins that narrate the review system or model when a direct assessment of the repo is enough
...
- prefer plain group names such as `core project docs`, `repo entry docs`, `setup and verification docs`, or `public docs` over framework-heavy labels when those names are enough
...
- recommend `validation/review/full-agent-surface-review.md` before the repo is treated as fully upgraded
...
- editing or rewriting the repo
- creating repo-local workflow artifacts for the upgrade process
...
1. A plain-language recommendation about the repo's current docs or public-docs setup.
...
- avoid lead-ins that narrate the review system itself when a direct statement about the repo is enough

For each main gap:

- describe the repo-surface problem in plain language first
...
- prefer repo-surface wording over internal architecture wording
- when code paths are cited, explain the repo-level consequence first
- avoid technical shorthand such as `architectural risk`, `stable facade`, `contract ownership`, or `validation portability` unless it is truly necessary to explain the repo-level issue
- avoid internal framework terms such as `source-of-truth`, `validation contract`, or `repo-local agents/` unless they materially help the user understand what to do next
- prefer phrases such as `architecture and testing docs`, `core project docs`, or `public docs` over framework-heavy labels such as `owner docs` or `stable owner docs`
- refer to `AGENTS.md` directly, or as the repo instructions, rather than as an `agent brief` when the plain file reference is enough
...
- explain what part of the repo the work covers
...
- only recommend a specific new supporting doc when current repo evidence clearly justifies it
